...
Requires numpy, scipy, matplotlib, and time packages along with KinetiKit \
package

Includes adjustments that should be made when the data spans a shorter period \
than the laser repetition rate
"""
...
to = sim.time.linear(N=10000, period=12.5*ns)
...
do = data.lib.data_from_SPCM(file_path, key = key,
                                    skip_h=153, skip_f=145,
                                    weigh_by_coll=True, metadata=False, coll=60)
...
conditions = fit.lib.sac_args(
        ...
        absolute=True,
        limits=[0*ns, 2.5*ns], # None or list of two time values
        norm=True,
